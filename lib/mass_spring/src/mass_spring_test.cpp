...
void readcmdline(int argc, char* argv[],std::string &file_path)
{
  size_t i;
  i=1;
  file_path=argv[i];
...
int main(int argc,char**argv)
{
  ...
  */
  ...
  readcmdline(argc,argv,file_path);
  boost::property_tree::read_json<boost::property_tree::ptree>(file_path,para_tree);

  para::out_dir_simulator=para_tree.get<string>("out_dir_simulator");
  printf("out_dir_simulator: %s\n",para::out_dir_simulator.c_str());
  para::simulation_type=para_tree.get<string>("simulation","static"); 
  para::newton_fastMS=para_tree.get<string>("solver");
  para::dt=para_tree.get<double>("delt_t",0.01);
  para::density=para_tree.get<double>("density",10);
  para::stiffness=para_tree.get<double>("stiffness",8000);
  para::frame=para_tree.get<int>("frames",3000);
  para::line_search=para_tree.get<int>("line_search",1);
  para::weight_line_search=para_tree.get<double>("weight_line_search",1e-5);
  para::gravity=para_tree.get<double>("gravity",9.8); 
  para::object_name=para_tree.get<string>("object_name"); 
  para::input_object=para_tree.get<string>("input_object");
  para::input_constraint=para_tree.get<string>("input_constraint");
  para::force_function=para_tree.get<string>("force_function");
  para::radius=para_tree.get<double>("radius");
  para::intensity=para_tree.get<double>("intensity");
